...
function Blogs() {
  ...
  return (
    <div className="article">
     ...
     <Section mt={6} py={0} >
       <Flex>
        <Box fontWeight={["400"]} pt={"8px"} lineHeight={1.6} color="gray.700" width={["100%","100%","65%"]}>
        {blog.content && blog.content.map((body, index) => (
            <Body {...body}></Body>
        ))}
        </Box>
        <Flex width={["100%","100%","35%"]} mt={6} display={["none","none","flex"]} alignItems="center" direction="column">
        {blog.logo && 
